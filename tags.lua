...
local function valShort(value)
	if(value >= 1e8) then
		return ("%.2f"):format(value / 1e8):gsub("%.?0+$", "") .. "y"
	elseif(value >= 1e5) then
		return ("%.1f"):format(value / 1e4):gsub("%.?0+$", "") .. "w"
...
oUF.Tags.Methods["threatBoss"] = function(unit)
	if GetNumGroupMembers() > 0 or GetNumPartyMembers() > 0 then
		local prefix, num = GetNumGroupMembers() > 0 and "raid" or "party", GetNumGroupMembers() > 0 and GetNumGroupMembers() or GetNumPartyMembers()
