...
local maxComboPoints = UnitPowerMax('player', SPELL_POWER_ENERGY)
...
oUF_Hank.UpdateStatus = function(self)
	...
	local icons = {
		...
		["A"] = {"Assistant", UnitInRaid("player") and UnitIsRaidOfficer("player") and not UnitIsGroupLeader("player")},
	}
...
function CheckFilter(filterType, isFriend)
...
oUF_Hank.euiFilter = function (icons, unit, icon, name, rank, texture, count, dtype, duration, timeLeft, unitCaster, isStealable, shouldConsolidate, spellID, canApplyAura, isBossAura)	
	if E.global.unitframe.InvalidSpells[spellID] then
		return false;
	end

	local isPlayer, isFriend
	local db = UF.db["units"]["target"][icons.type]
	-- if not db or not db[self.type] then return true; end

	local returnValue = true
	local passPlayerOnlyCheck = true
	local anotherFilterExists = false
	local isPlayer = unitCaster == 'player' or unitCaster == 'vehicle' or unitCaster == 'pet'
	...
	if CheckFilter(db.onlyDispellable, isFriend) then
		if (icons.type == 'buffs' and not isStealable) or (icons.type == 'debuffs' and dtype and  not E:IsDispellableByMe(dtype)) or dtype == nil then
			returnValue = false;
		end
		anotherFilterExists = true
	end
...
oUF_Hank.sharedStyle = function(self, unit, isSingle)
	...
	-- Auras
	if unit == "target" or unit == "focus" or unit:find("boss") then
		-- Buffs
		...
		self.Buffs.CustomFilter = oUF_Hank.euiFilter
		self.Debuffs.CustomFilter = oUF_Hank.euiFilter
	end
		-- Auras
	-- if string.sub(unit,1,4) == "boss" then
	-- 	-- -- Buffs
	-- 	-- self.Buffs = CreateFrame("Frame", unit .. "_Buffs", self) -- ButtonFace needs a name
	-- 	-- if self.CPoints then
	-- 	-- 	self.Buffs:SetPoint("TOPLEFT", self.CPoints[1], "BOTTOMLEFT", 0, -5)
	...
	if cfg.Castbar and (unit == "player" or unit == "target" or unit == "focus") then
		...
		elseif unit == "focus" then
			...
			cb:SetPoint("RIGHT", self, "LEFT", -10 - cfg.CastbarFocusMargin[1], cfg.CastbarFocusMargin[2])
		else
